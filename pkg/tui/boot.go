...
// Boot styles
var (
	bootTitleGradient = []string{"#FF79C6", "#BD93F9", "#8BE9FD"}
...
func (m BootModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case bootTickMsg:
		m.animFrame = (m.animFrame + 1) % len(waveFrames)
...
func (m BootModel) View() string {
	...
	// Animated wave
	wave := bootWaveStyle.Render(waveFrames[m.animFrame%len(waveFrames)])
	b.WriteString(wave)
	b.WriteString("\n\n")
	...
	colorIdx := m.animFrame % len(bootTitleGradient)
	...
	if total > 0 {
		prog := float64(completed) / float64(total)
	...
	b.WriteString(bootBoxBorder.Render(servicesContent))
	b.WriteString("\n")
