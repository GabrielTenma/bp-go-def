...
func NewMongoDB(cfg config.MongoConfig) (*MongoManager, error) {
	if !cfg.Enabled {
		return nil, nil
	}
...
func NewMongoConnectionManager(cfg config.MongoMultiConfig) (*MongoConnectionManager, error) {
	if !cfg.Enabled {
		return nil, nil
	}
